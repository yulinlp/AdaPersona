...
5. Validate and execute candidates in disposable processes. The failure bank records fitting errors and regressions only. Rank candidates by historical selection score, then fitting score. Acceptance requires zero errors, a **strict improvement on hidden historical selection**, and no fitting regression. Fitting-only gains with tied selection are not accepted: memorizing fitting answers must not earn promotion. Perfect fitting accuracy may stay tied while selection improves. Fresh candidate/parent rechecks must reproduce predictions and call traces on both partitions. Otherwise keep the incumbent. Valid non-promoted programs remain available for exploration. Selection examples, predictions and detailed scores are never passed to the Code Agent.
...
Historical source identity uses task content, ignoring IDs and auxiliary metadata and normalizing whitespace. Copies cannot bypass own-source exclusion or enter fitting profiles through a held-out record with a different ID. Records with identical task inputs are grouped before splitting; the first source-file reference represents the group, and all member records are excluded together from evaluation profiles. Preparation and evolution validate source separation; identical task inputs across partitions fail closed. This is exact-content protection, not semantic near-duplicate detection.

Before execution, candidates are screened for complete fitting references, long verbatim excerpts (20 consecutive words), exact sample-ID lookups and complete long input lookups. Constant string concatenation and literal-list joins are inspected without executing code. Short category labels, rating values and style phrases remain allowed. These checks are conservative heuristics, not proof against encoded answers or arbitrary memorization. Runtime reference removal and disposable-process I/O restrictions remain separate defenses; the Python restrictions are not a security-grade sandbox for adversarial code.

Hidden selection must show a strict gain even if fitting becomes perfect. Small historical selection sets still provide weak evidence, and repeated selection can overfit them. Independent-fold evolution and a third, untouched historical audit partition are **not implemented**; do not label current runs cross-validated. Final scientific claims require untouched evaluation users, especially if monitored test results informed method development. The new admission protocol requires a fresh output directory and is not retroactively applied to old runs.
